#!/usr/bin/env python3
"""
Debug version of gemini_extract.py for Docker troubleshooting
"""
import argparse
import io
import json
...
async def gemini_extract(
    ...
    print(f"DEBUG: Starting gemini_extract with:")
    print(f"DEBUG: file_path = {file_path}")
    print(f"DEBUG: project = {project}")
    print(f"DEBUG: GOOGLE_CLOUD_PROJECT env = {os.getenv('GOOGLE_CLOUD_PROJECT')}")
    print(f"DEBUG: location = {location}")
    print(f"DEBUG: GOOGLE_CLOUD_LOCATION env = {os.getenv('GOOGLE_CLOUD_LOCATION')}")
    print(f"DEBUG: USE_VERTEXAI = {USE_VERTEXAI}")
    ...
    if os.path.exists(os.path.expanduser("~/.config/gcloud/application_default_credentials.json")):
        print(f"DEBUG: Default credentials file exists")
    else:
        print(f"DEBUG: Default credentials file does not exist")
    ...
    try:
        ...
        if effective_project is None:
            print("ERROR: No Google Cloud project specified. Set GOOGLE_CLOUD_PROJECT environment variable or pass --project")
            return []
        ...
        print(f"DEBUG: Created genai client with project={effective_project}, location={location}")

        image_bytes = read_image_as_jpeg_bytes(file_path)
        print(f"DEBUG: Read image as JPEG bytes, size={len(image_bytes)} bytes")
        ...
        print(f"DEBUG ERROR in gemini_extract: {str(e)}")
